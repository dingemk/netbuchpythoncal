...
exports.createMpFsBuilderCache = createMpFsBuilderCache;
exports.addIntelHexFiles = addIntelHexFiles;
exports.generateHexWithFiles = generateHexWithFiles;
exports.calculateFileSize = calculateFileSize;
exports.getIntelHexFiles = getIntelHexFiles;
exports.getMemMapFsSize = getMemMapFsSize;
/**
...
function createMpFsBuilderCache(originalIntelHex) {
    const originalMemMap = nrf_intel_hex_1.default.fromHex(originalIntelHex);
    const deviceMem = (0, hex_mem_info_js_1.getHexMapDeviceMemInfo)(originalMemMap);
...
function getStartAddress(intelHexMap) {
    const deviceMem = (0, hex_mem_info_js_1.getHexMapDeviceMemInfo)(intelHexMap);
...
function getEndAddress(intelHexMap) {
    ...
    if (deviceMem.deviceVersion === 'V1') {
        if ((0, micropython_appended_js_1.isAppendedScriptPresent)(intelHexMap)) {
            endAddress = micropython_appended_js_1.AppendedBlock.StartAdd;
        }
...
class FsFile {
    ...
    constructor(filename, data) {
        ...
        this._filenameBytes = (0, common_js_1.strToBytes)(filename);
...
function addIntelHexFiles(intelHex, files, returnBytes = false) {
    ...
    if (typeof intelHex === 'string') {
        intelHexMap = nrf_intel_hex_1.default.fromHex(intelHex);
    }
    else {
...
function getIntelHexFiles(intelHex) {
    let hexMap;
    if (typeof intelHex === 'string') {
        hexMap = nrf_intel_hex_1.default.fromHex(intelHex);
    }
    else {
    ...
    for (const startChunkIndex of startChunkIndexes) {
        ...
        const filename = (0, common_js_1.bytesToStr)(startChunk.slice(3, chunkDataStart));
        ...
        while (iterations--) {
            ...
            if (nextIndex === ChunkMarker.Unused) {
                ...
                files[filename] = (0, common_js_1.concatUint8Array)(files[filename], currentChunk.slice(chunkDataStart, 1 + endChunkOffset));
                break;
            }
            else {
                files[filename] = (0, common_js_1.concatUint8Array)(files[filename], currentChunk.slice(chunkDataStart, ChunkFormatIndex.Tail));
            }
