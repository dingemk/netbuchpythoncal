...
const micropython_fs_builder_js_1 = require("../micropython-fs-builder.js");
const uPy1HexFile = fs.readFileSync('./src/__tests__/upy-v1.0.1.hex', 'utf8');
const uPy1HexMap = nrf_intel_hex_1.default.fromHex(uPy1HexFile);
...
const uPy2HexMap = nrf_intel_hex_1.default.fromHex(uPy2HexFile);
const uPy2FsSize = 20 * 1024;
const uPy2FsStart = 0x6d000;
const makecodeHexFile = fs.readFileSync('./src/__tests__/makecode.hex', 'utf8');
const randContent = (0, common_js_1.strToBytes)('Some random content.');
(0, vitest_1.describe)('Writing files to the filesystem.', () => {
    const files = [
        {
            ...
            fsSize: 128,
            bytes() {
                return nrf_intel_hex_1.default.fromHex(this.hex).get(this.fileAddress);
            },
        },
        {
    ...
    (0, vitest_1.it)('Add files to hex, return a string (uPy1)', () => {
        const fwWithFsOther = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexFile, {
            [files[0].fileName]: (0, common_js_1.strToBytes)(files[0].fileStr),
            [files[1].fileName]: (0, common_js_1.strToBytes)(files[1].fileStr),
        });
        const opMap = nrf_intel_hex_1.default.fromHex(fwWithFsOther);
        ...
        (0, vitest_1.expect)(file0data).toEqual(files[0].bytes());
        (0, vitest_1.expect)(file1data).toEqual(files[1].bytes());
    });
    (0, vitest_1.it)('Add files to hex, return a string (uPy2)', () => {
        const fwWithFsOther = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy2HexFile, {
            [files[0].fileName]: (0, common_js_1.strToBytes)(files[0].fileStr),
            [files[1].fileName]: (0, common_js_1.strToBytes)(files[1].fileStr),
        });
        const opMap = nrf_intel_hex_1.default.fromHex(fwWithFsOther);
    ...
    (0, vitest_1.it)('Add files to hex, return a byte array (uPy1)', () => {
        const fwWithFsBytes = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexFile, {
            [files[0].fileName]: (0, common_js_1.strToBytes)(files[0].fileStr),
            [files[1].fileName]: (0, common_js_1.strToBytes)(files[1].fileStr),
        }, true);
        const file0data = fwWithFsBytes.slice(uPy1FsStart, uPy1FsStart + files[0].fsSize);
        const file1start = uPy1FsStart + files[0].fsSize;
        const file1data = fwWithFsBytes.slice(file1start, file1start + files[1].fsSize);
        (0, vitest_1.expect)(file0data).toEqual(files[0].bytes());
        (0, vitest_1.expect)(file1data).toEqual(files[1].bytes());
    });
    (0, vitest_1.it)('Add files to hex, return a byte array (uPy2)', () => {
        const fwWithFsBytes = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy2HexFile, {
            [files[0].fileName]: (0, common_js_1.strToBytes)(files[0].fileStr),
            [files[1].fileName]: (0, common_js_1.strToBytes)(files[1].fileStr),
        }, true);
        const file0data = fwWithFsBytes.slice(uPy2FsStart, uPy2FsStart + files[0].fsSize);
        const file1start = uPy2FsStart + files[0].fsSize;
        const file1data = fwWithFsBytes.slice(file1start, file1start + files[1].fsSize);
        (0, vitest_1.expect)(file0data).toEqual(files[0].bytes());
        (0, vitest_1.expect)(file1data).toEqual(files[1].bytes());
    });
    (0, vitest_1.it)('Input to addIntelHexFiles() as a Intel Hex string and Memory Map generate compatible data (uPy1)', () => {
        const fsFiles = {
            [files[0].fileName]: (0, common_js_1.strToBytes)(files[0].fileStr),
            [files[1].fileName]: (0, common_js_1.strToBytes)(files[1].fileStr),
        };
        for (let i = 0; i < 32; i++) {
            fsFiles['file_' + i + '.txt'] = (0, common_js_1.strToBytes)("Content doesn't matter " + i);
        }
        const hexFromIntelHexV1 = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexFile, fsFiles);
        const hexFromMemoryMapV1 = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, fsFiles);
        const hexFromIntelHexV2 = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy2HexFile, fsFiles);
        const hexFromMemoryMapV2 = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy2HexMap, fsFiles);
        (0, vitest_1.expect)(hexFromIntelHexV1).toEqual(hexFromMemoryMapV1);
        (0, vitest_1.expect)(hexFromIntelHexV2).toEqual(hexFromMemoryMapV2);
    });
    (0, vitest_1.it)('Both addIntelHexFiles() and generateHexWithFiles() generate compatible data', () => {
        const fsFiles = {
            [files[0].fileName]: (0, common_js_1.strToBytes)(files[0].fileStr),
            [files[1].fileName]: (0, common_js_1.strToBytes)(files[1].fileStr),
        };
        ...
        const cache = (0, micropython_fs_builder_js_1.createMpFsBuilderCache)(uPy1HexFile);
        const hexFromIntelHex = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexFile, fsFiles);
        const hexFromCache = (0, micropython_fs_builder_js_1.generateHexWithFiles)(cache, fsFiles);
        ...
        const hexFromIntelHexPlusRecord = hexFromIntelHex.replace(':0888B00095880100C1000000E1', ':0888B00095880100C1000000E1\n:020000040003F7');
        (0, vitest_1.expect)(hexFromCache).toEqual(hexFromIntelHexPlusRecord);
    });
    ...
    (0, vitest_1.it)('Can generate a full chunk that also consumes the next one.', () => {
        const fwWithFsOther = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, {
            [fullChunkPlus.fileName]: (0, common_js_1.strToBytes)(fullChunkPlus.fileStr),
        });
        const opMap = nrf_intel_hex_1.default.fromHex(fwWithFsOther);
        const readFileData = opMap
            .slice(fullChunkPlus.fileAddress, fullChunkPlus.fsSize)
            .get(fullChunkPlus.fileAddress);
        (0, vitest_1.expect)(readFileData).toEqual(fullChunkPlus.bytes());
    });
    (0, vitest_1.it)('Correctly generate an almost full chunk (not using last byte).', () => {
        const fwWithFsOther = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, {
            [fullChunkMinus.fileName]: (0, common_js_1.strToBytes)(fullChunkMinus.fileStr),
        });
        const opMap = nrf_intel_hex_1.default.fromHex(fwWithFsOther);
        const readFileData = opMap
            .slice(fullChunkMinus.fileAddress, fullChunkMinus.fsSize)
            .get(fullChunkMinus.fileAddress);
        (0, vitest_1.expect)(readFileData).toEqual(fullChunkMinus.bytes());
    });
    (0, vitest_1.it)('Correctly generate just over a full chunk.', () => {
        const fwWithFsOther = (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, {
            [twoChunks.fileName]: (0, common_js_1.strToBytes)(twoChunks.fileStr),
        });
        const opMap = nrf_intel_hex_1.default.fromHex(fwWithFsOther);
        const readFileData = opMap
            .slice(twoChunks.fileAddress, twoChunks.fsSize)
            .get(twoChunks.fileAddress);
        (0, vitest_1.expect)(readFileData).toEqual(twoChunks.bytes());
    });
    (0, vitest_1.it)('Empty file name throws an error.', () => {
        (0, vitest_1.expect)(() => (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, { '': randContent })).toThrow('File has to have a file name');
        (0, vitest_1.expect)(() => (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy2HexMap, { '': randContent })).toThrow('File has to have a file name');
    });
    (0, vitest_1.it)('Empty file data throw an error.', () => {
        (0, vitest_1.expect)(() => (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, { 'my_file.txt': new Uint8Array(0) })).toThrow('has to contain data');
        (0, vitest_1.expect)(() => (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy2HexMap, { 'my_file.txt': new Uint8Array(0) })).toThrow('has to contain data');
    });
    (0, vitest_1.it)('Large file that does not fit throws error.', () => {
        (0, vitest_1.expect)(() => {
            (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, {
                'my_file.txt': new Uint8Array(50 * 1024).fill(0x55),
            });
        }).toThrow('Not enough space');
        (0, vitest_1.expect)(() => {
            (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy2HexMap, {
                'my_file.txt': new Uint8Array(50 * 1024).fill(0x55),
            });
        }).toThrow('Not enough space');
    });
    (0, vitest_1.it)('Add files until no more fit.', () => {
        ...
        const addLargeFiles = () => {
            ...
            for (let i = 0; i < 15; i++) {
                hexWithFs = (0, micropython_fs_builder_js_1.addIntelHexFiles)(hexWithFs, {
        ...
        (0, vitest_1.expect)(addLargeFiles).toThrow('Not enough space');
        (0, vitest_1.expect)(completeFsFilling).toThrow('no storage space left');
    });
    (0, vitest_1.it)('Add a group of files that do not fit.', () => {
        const test = (memMap, fsSize) => {
            // Use 4 KB blocks per file (each 128 Byte chunk has 126 bytes data)
            const fakeBigFileData = new Uint8Array(4000).fill(0x55);
            const tooManyBigFiles = {};
            for (let i = 0; i < fsSize / 4096 + 1; i++) {
                tooManyBigFiles['file_' + i + '.txt'] = fakeBigFileData;
            }
            const addingAllFiles = () => (0, micropython_fs_builder_js_1.addIntelHexFiles)(memMap, tooManyBigFiles);
            (0, vitest_1.expect)(addingAllFiles).toThrow('space');
        };
        test(uPy1HexMap, uPy1FsSize);
        test(uPy2HexMap, uPy2FsSize);
    });
    (0, vitest_1.it)('Max filename length works.', () => {
        const maxLength = 120;
        const largeName = 'a'.repeat(maxLength);
        const workingCase = () => (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, { [largeName]: randContent });
        (0, vitest_1.expect)(workingCase).not.toThrow(Error);
    });
    (0, vitest_1.it)('Too large filename throws error.', () => {
        const maxLength = 120;
        const largeName = 'a'.repeat(maxLength + 1);
        const failCase = () => (0, micropython_fs_builder_js_1.addIntelHexFiles)(uPy1HexMap, { [largeName]: randContent });
        (0, vitest_1.expect)(failCase).toThrow('File name');
    });
    (0, vitest_1.it)('Adding files to non-MicroPython hex fails.', () => {
        const failCase = () => (0, micropython_fs_builder_js_1.addIntelHexFiles)(makecodeHexFile, { 'a.py': randContent });
        (0, vitest_1.expect)(failCase).toThrow('Could not find valid MicroPython UICR');
    });
    // TODO: Hex file with persistent page marker doesn't get two markers
    // TODO: Create tests for createMpFsBuilderCache
});
(0, vitest_1.describe)('Reading files from the filesystem.', () => {
    const alastFilename = 'alast.py';
    const alastContent = (0, common_js_1.strToBytes)('# Lorem Ipsum is simply dummy text of the printing and\n' +
        "# typesetting industry. Lorem Ipsum has been the industry's\n" +
        '# standard dummy text ever since the 1500s, when an unknown\n' +
        '# printer took a galley of type and scrambled it to make a\n' +
        '# type specimen book. It has survived not only five\n' +
    ...
    const afirstContent = (0, common_js_1.strToBytes)("firstname = 'Carlos'");
    // Uses chunk ??
    const afirstHex = ':020000040003F7\n' +
        ':10DF0000FE1F096166697273742E70796669727397\n' +
        ':10DF1000746E616D65203D20274361726C6F7327BD\n' +
        ':00000001FF\n';
    const mainFilename = 'main.py';
    const mainContent = (0, common_js_1.strToBytes)('from microbit import display, Image, sleep\n\n' +
    ...
    const mainHex = ':020000040003F7\n' +
        ...
        ':00000001FF\n';
    (0, vitest_1.it)('Can read files of different sizes in non-consecutive locations.', () => {
        const addHexToMap = (hexMap, hex) => {
            const newMemMap = nrf_intel_hex_1.default.fromHex(hex);
            newMemMap.forEach((value, index) => {
        ...
        const foundFilesFromHex = (0, micropython_fs_builder_js_1.getIntelHexFiles)(fullUpyFsMemMap.asHexString());
        const foundFilesFromMap = (0, micropython_fs_builder_js_1.getIntelHexFiles)(fullUpyFsMemMap);
        (0, vitest_1.expect)(foundFilesFromHex).toEqual(foundFilesFromMap);
        (0, vitest_1.expect)(foundFilesFromHex).toHaveProperty([afirstFilename], afirstContent);
        (0, vitest_1.expect)(foundFilesFromHex).toHaveProperty([alastFilename], alastContent);
        (0, vitest_1.expect)(foundFilesFromHex).toHaveProperty([mainFilename], mainContent);
    });
    ...
    const oneChunkPlusContent = 'a = """abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmno"""\n';
    // Uses chunk 0x45 and 0x46
    const oneChunkPlusHex = ':020000040003F7\n' +
        ...
        ':00000001FF\n';
    (0, vitest_1.it)('Can read a file that occupies a single chunk and also the next.', () => {
        ...
        const fullUpyFsMemMap = uPy1HexMap.clone();
        const oneChunkPlusMemMap = nrf_intel_hex_1.default.fromHex(oneChunkPlusHex);
        oneChunkPlusMemMap.forEach((value, index) => {
            fullUpyFsMemMap.set(index, value);
        });
        const foundFilesFromHex = (0, micropython_fs_builder_js_1.getIntelHexFiles)(fullUpyFsMemMap.asHexString());
        const foundFilesFromMap = (0, micropython_fs_builder_js_1.getIntelHexFiles)(fullUpyFsMemMap);
        (0, vitest_1.expect)(foundFilesFromHex).toEqual(foundFilesFromMap);
        (0, vitest_1.expect)(foundFilesFromHex).toHaveProperty([oneChunkPlusFilename], (0, common_js_1.strToBytes)(oneChunkPlusContent));
    });
    const oneChunkMinusFilename = 'one_chunk_minus.py';
    const oneChunkMinusContent = 'a = """abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklmnopqrstuvwxyz\n' +
        'abcdefghijklm"""\n';
    // Uses chunk ??
    const oneChunkMinusHex = ':020000040003F7\n' +
        ...
        ':00000001FF\n';
    (0, vitest_1.it)('Can read a file that occupies almost a full chunk.', () => {
        ...
        const fullUpyFsMemMap = uPy1HexMap.clone();
        const oneChunkMinusMemMap = nrf_intel_hex_1.default.fromHex(oneChunkMinusHex);
        oneChunkMinusMemMap.forEach((value, index) => {
            fullUpyFsMemMap.set(index, value);
        });
        const foundFilesFromHex = (0, micropython_fs_builder_js_1.getIntelHexFiles)(fullUpyFsMemMap.asHexString());
        const foundFilesFromMap = (0, micropython_fs_builder_js_1.getIntelHexFiles)(fullUpyFsMemMap);
        (0, vitest_1.expect)(foundFilesFromHex).toEqual(foundFilesFromMap);
        (0, vitest_1.expect)(foundFilesFromHex).toHaveProperty([oneChunkMinusFilename], (0, common_js_1.strToBytes)(oneChunkMinusContent));
    });
    // Uses chunks ??
    const oneFileCopyHex = ':020000040003F7\n' +
        ':10AE0000FE1804612E707961203D20274A75737405\n' +
        ':10AE100020612066696C65270AFFFFFFFFFFFFFFC7\n' +
        ':00000001FF\n';
    const otherFileCopyHex = ':020000040003F7\n' +
        ':10C18000FE1804612E707961203D20274A75737472\n' +
        ':10C1900020612066696C65270AFFFFFFFFFFFFFF34\n' +
        ':00000001FF\n';
    (0, vitest_1.it)('Duplicate file names throws an error.', () => {
        // In contrast to the one_chunk_plus.py example, this one fills the chunk
        // minus 1 Byte (The second 0xFF at the end is the chunk tail).
        const fullUpyFsMemMap = uPy1HexMap.clone();
        const oneFileCopyMemMap = nrf_intel_hex_1.default.fromHex(oneFileCopyHex);
        oneFileCopyMemMap.forEach((value, index) => {
            fullUpyFsMemMap.set(index, value);
        });
        const otherFileCopyMemMap = nrf_intel_hex_1.default.fromHex(otherFileCopyHex);
        otherFileCopyMemMap.forEach((value, index) => {
            fullUpyFsMemMap.set(index, value);
        });
        const failCase = () => (0, micropython_fs_builder_js_1.getIntelHexFiles)(fullUpyFsMemMap.asHexString());
        (0, vitest_1.expect)(failCase).toThrow('Found multiple files named');
    });
    (0, vitest_1.it)('Reading files from empty MicroPython hex returns empty list.', () => {
        (0, vitest_1.expect)((0, micropython_fs_builder_js_1.getIntelHexFiles)(uPy1HexMap)).toEqual({});
        (0, vitest_1.expect)((0, micropython_fs_builder_js_1.getIntelHexFiles)(uPy2HexMap)).toEqual({});
    });
    (0, vitest_1.it)('Reading files from non-MicroPython hex fails.', () => {
        const failCase = () => (0, micropython_fs_builder_js_1.getIntelHexFiles)(makecodeHexFile);
        (0, vitest_1.expect)(failCase).toThrow('Could not find valid MicroPython UICR');
    });
...
(0, vitest_1.describe)('Calculate sizes.', () => {
    (0, vitest_1.it)('Get how much available fs space there is in a MicroPython hex file.', () => {
        (0, vitest_1.expect)((0, micropython_fs_builder_js_1.getMemMapFsSize)(uPy1HexMap)).toEqual(uPy1FsSize);
        (0, vitest_1.expect)((0, micropython_fs_builder_js_1.getMemMapFsSize)(uPy2HexMap)).toEqual(uPy2FsSize);
    });
    (0, vitest_1.it)('Calculate the space occupied for a file in the fs.', () => {
        const fileSizeOne = (0, micropython_fs_builder_js_1.calculateFileSize)('one_chunk.txt', new Uint8Array([30, 31, 32, 33, 34]));
        const fileSizeAlmostFullOne = (0, micropython_fs_builder_js_1.calculateFileSize)('almost_one_chunk____.txt', new Uint8Array(99).fill(0x35));
        const fileSizeOneOverflow = (0, micropython_fs_builder_js_1.calculateFileSize)('one_chunk_overflow__.txt', new Uint8Array(100).fill(0x35));
        const fileSizeTwo = (0, micropython_fs_builder_js_1.calculateFileSize)('just_about_2_chunks_.txt', new Uint8Array(101).fill(0x35));
        const fileSizeNine = (0, micropython_fs_builder_js_1.calculateFileSize)('9_chunks.txt', new Uint8Array(1100).fill(0x35));
        // Real size counts chunks, so always a multiple of 128
        (0, vitest_1.expect)(fileSizeOne).toEqual(128);
        (0, vitest_1.expect)(fileSizeAlmostFullOne).toEqual(128);
        (0, vitest_1.expect)(fileSizeOneOverflow).toEqual(128 * 2);
        (0, vitest_1.expect)(fileSizeTwo).toEqual(128 * 2);
        (0, vitest_1.expect)(fileSizeNine).toEqual(128 * 9);
    });
});
