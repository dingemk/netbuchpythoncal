...
describe('Test the class constructor', () => {
    ...
    it('Invalid hex for MicroPython throws an error', () => {
        expect(() => new MicropythonFsHex('nothex')).toThrow('Malformed .hex file, could not parse any registers');
        expect(() => {
            new MicropythonFsHex([
                {
                    hex: 'notahex',
...
describe('Test general read write operations', () => {
    it('Write and read files', () => {
        ...
        testInstance(new MicropythonFsHex([
...
describe('Test size operations.', () => {
    ...
    it('The maximum filesystem size cannot be larger than space available', () => {
        expect(() => new MicropythonFsHex(uPy1HexFile, { maxFsSize: 1024 * 1024 })).toThrow('Storage size limit provided is larger than size available');
        expect(() => new MicropythonFsHex([
            { hex: uPy1HexFile, boardId: 0x9900 },
            { hex: uPy2HexFile, boardId: 0x9903 },
...
describe('Test importing files from Intel Hex.', () => {
    ...
    it('Constructor hex file with files to import throws an error.', () => {
        expect(() => new MicropythonFsHex(hexStrWithFiles)).toThrow('There are files in the MicropythonFsHex constructor hex file input');
        expect(() => new MicropythonFsHex([{ hex: hexStrWithFiles, boardId: 0x9903 }])).toThrow('There are files in the MicropythonFsHex constructor hex file input');
    });
...
describe('End-to-end loop around.', () => {
    it('Create some files, export Intel hex, and import in a new instance', () => {
        ...
        expect(microbitFsImported.readBytes('a.txt')).toEqual(microbitFs.readBytes('a.txt'));
        expect(microbitFsImported.size('a.txt')).toEqual(microbitFs.size('a.txt'));
        expect(microbitFsImported.getStorageUsed()).toEqual(microbitFs.getStorageUsed());
    });
    it('Create some files, export Universal hex, and import in a new instance', () => {
        ...
        expect(microbitFsImported.size('a.txt')).toEqual(microbitFs.size('a.txt'));
        expect(microbitFsImported.getStorageUsed()).toEqual(microbitFs.getStorageUsed());
        expect(microbitFsImported.getIntelHex(microbitBoardId.V1)).toEqual(microbitFs.getIntelHex(microbitBoardId.V1));
        expect(microbitFsImported.getIntelHex(microbitBoardId.V2)).toEqual(microbitFs.getIntelHex(microbitBoardId.V2));
    });
});
/*
