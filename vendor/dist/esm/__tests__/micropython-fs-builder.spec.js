...
import { createMpFsBuilderCache, generateHexWithFiles, addIntelHexFiles, calculateFileSize, getIntelHexFiles, getMemMapFsSize, } from '../micropython-fs-builder.js';
...
describe('Writing files to the filesystem.', () => {
    ...
    it('Add files to hex, return a byte array (uPy1)', () => {
        const fwWithFsBytes = addIntelHexFiles(uPy1HexFile, {
            [files[0].fileName]: strToBytes(files[0].fileStr),
            [files[1].fileName]: strToBytes(files[1].fileStr),
        }, true);
    ...
    it('Add files to hex, return a byte array (uPy2)', () => {
        const fwWithFsBytes = addIntelHexFiles(uPy2HexFile, {
            [files[0].fileName]: strToBytes(files[0].fileStr),
            [files[1].fileName]: strToBytes(files[1].fileStr),
        }, true);
    ...
    it('Input to addIntelHexFiles() as a Intel Hex string and Memory Map generate compatible data (uPy1)', () => {
        ...
        const hexFromIntelHexV1 = addIntelHexFiles(uPy1HexFile, fsFiles);
        const hexFromMemoryMapV1 = addIntelHexFiles(uPy1HexMap, fsFiles);
        const hexFromIntelHexV2 = addIntelHexFiles(uPy2HexFile, fsFiles);
        const hexFromMemoryMapV2 = addIntelHexFiles(uPy2HexMap, fsFiles);
    ...
    it('Both addIntelHexFiles() and generateHexWithFiles() generate compatible data', () => {
        ...
        const hexFromIntelHex = addIntelHexFiles(uPy1HexFile, fsFiles);
    ...
    it('Empty file name throws an error.', () => {
        expect(() => addIntelHexFiles(uPy1HexMap, { '': randContent })).toThrow('File has to have a file name');
        expect(() => addIntelHexFiles(uPy2HexMap, { '': randContent })).toThrow('File has to have a file name');
    });
    it('Empty file data throw an error.', () => {
        expect(() => addIntelHexFiles(uPy1HexMap, { 'my_file.txt': new Uint8Array(0) })).toThrow('has to contain data');
        expect(() => addIntelHexFiles(uPy2HexMap, { 'my_file.txt': new Uint8Array(0) })).toThrow('has to contain data');
    });
    ...
    it('Max filename length works.', () => {
        ...
        const workingCase = () => addIntelHexFiles(uPy1HexMap, { [largeName]: randContent });
        expect(workingCase).not.toThrow(Error);
    });
    it('Too large filename throws error.', () => {
        const maxLength = 120;
        const largeName = 'a'.repeat(maxLength + 1);
        const failCase = () => addIntelHexFiles(uPy1HexMap, { [largeName]: randContent });
        expect(failCase).toThrow('File name');
    });
    it('Adding files to non-MicroPython hex fails.', () => {
        const failCase = () => addIntelHexFiles(makecodeHexFile, { 'a.py': randContent });
...
describe('Reading files from the filesystem.', () => {
    ...
    const alastContent = strToBytes('# Lorem Ipsum is simply dummy text of the printing and\n' +
        "# typesetting industry. Lorem Ipsum has been the industry's\n" +
        '# standard dummy text ever since the 1500s, when an unknown\n' +
        '# printer took a galley of type and scrambled it to make a\n' +
        '# type specimen book. It has survived not only five\n' +
    ...
    const mainContent = strToBytes('from microbit import display, Image, sleep\n\n' +
    ...
    it('Can read a file that occupies a single chunk and also the next.', () => {
        ...
        const foundFilesFromMap = getIntelHexFiles(fullUpyFsMemMap);
        expect(foundFilesFromHex).toEqual(foundFilesFromMap);
        expect(foundFilesFromHex).toHaveProperty([oneChunkPlusFilename], strToBytes(oneChunkPlusContent));
    });
    ...
    it('Can read a file that occupies almost a full chunk.', () => {
        ...
        const foundFilesFromMap = getIntelHexFiles(fullUpyFsMemMap);
        expect(foundFilesFromHex).toEqual(foundFilesFromMap);
        expect(foundFilesFromHex).toHaveProperty([oneChunkMinusFilename], strToBytes(oneChunkMinusContent));
    });
    // Uses chunks ??
...
describe('Calculate sizes.', () => {
    ...
    it('Calculate the space occupied for a file in the fs.', () => {
        const fileSizeOne = calculateFileSize('one_chunk.txt', new Uint8Array([30, 31, 32, 33, 34]));
        const fileSizeAlmostFullOne = calculateFileSize('almost_one_chunk____.txt', new Uint8Array(99).fill(0x35));
        const fileSizeOneOverflow = calculateFileSize('one_chunk_overflow__.txt', new Uint8Array(100).fill(0x35));
        const fileSizeTwo = calculateFileSize('just_about_2_chunks_.txt', new Uint8Array(101).fill(0x35));
        const fileSizeNine = calculateFileSize('9_chunks.txt', new Uint8Array(1100).fill(0x35));
