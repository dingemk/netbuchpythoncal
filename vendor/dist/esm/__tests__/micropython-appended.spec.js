...
import { addIntelHexAppendedScript, getIntelHexAppendedScript, isAppendedScriptPresent, } from '../micropython-appended.js';
...
describe('Inject Python code into Intel Hex string', () => {
    it('Inject Python code into an Intel Hex string', () => {
        const output = addIntelHexAppendedScript(simpleIntelHex, pyCode);
    ...
    it('Inject Python with present UICR and Start Linear Address record', () => {
        ...
        const output = addIntelHexAppendedScript(fullHex.join('\n'), pyCode);
    ...
    it('Inject Python in a hex with a  marker', () => {
        ...
        const expectedHex = simpleIntelHex.split('\n');
        expectedHex.splice(2, 0, pyCodeHex);
        const outputWithMarker = addIntelHexAppendedScript(fullHexWithMarker.join('\n'), pyCode);
        const outputWithout = addIntelHexAppendedScript(fullHexWithout.join('\n'), pyCode);
...
describe('Extract Python code from Intel Hex string', () => {
    it('Extract Python code', () => {
        ...
        const intelHex2 = ':020000040000FA\n' +
            ...
            ':00000001FF\n';
        const result1 = getIntelHexAppendedScript(intelHex1);
        const result2 = getIntelHexAppendedScript(intelHex2);
    ...
    it('Extract Python code with present UICR and Start Linear Address record)', () => {
        const intelHex = ':020000040000FA\n' +
            ...
            ':0400000500018E2147\n' +
            ':00000001FF\n';
        const result = getIntelHexAppendedScript(intelHex);
...
describe('Detect appended script.', () => {
    it('Appended script can be detected.', () => {
        const outputHex = addIntelHexAppendedScript(simpleIntelHex, 'code');
        const outputMap = MemoryMap.fromHex(outputHex);
    ...
    it('Missing appended script can be detected.', () => {
        const simpleMap = MemoryMap.fromHex(simpleIntelHex);
